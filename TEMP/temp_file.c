...
/*
 	A handle for a termporary file created with write_temp_file.
...
temp_file_handle write_temp_file(char* buffer, size_t length){
	/*
		...
		The XXXXXX will be replaced with
		chaacters that make the filename unique.
	 */
	char temp_filename[] = "/tmp/temp_file.XXXXXX";
	int fd = mkstemp(temp_filename);
	/*
	 	Unlink the file immediately, 
		so that it will be removed when the
	...
	/*
	 	Use the file descriptor as the handle
